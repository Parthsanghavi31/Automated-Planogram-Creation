...
def fit_products_into_bins(products, bin_max_width):
    ...
    for product in sorted(products, key=lambda x: x.height, reverse=True):  # Keep your existing sort if it aligns with your needs.
        placed = False
        for bin in bins:
        ...
        if not placed:
            ...
            bins.append(new_bin)  # Make sure new bin is added regardless of product fit
    return bins    
